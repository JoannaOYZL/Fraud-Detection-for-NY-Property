# Perform variable construction on dataset 3
library(dplyr)
library(stringr)
library(ggplot2)
data=read.csv("cleaned_data_v3.csv")

## Create ZIP3: the first 3 digits of zip code
data$ZIP3=substr(data$ZIP,1,3)
...
## create 45 variables: Group 9 ratio variables by ZIP5, ZIP3, BOROUGH, TAXCLASS, and them ALL
data2 = data1%>%
  group_by(ZIP) %>%
...
# divide ratio variables by their average values
data = data3
data = data %>%
