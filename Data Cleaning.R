...
library(stringr)
# load data set
dat=read_excel("NY property 1 million.xlsx")
str(dat)
dat$BBLE<-as.character(dat$BBLE)

# transform values of variable "borough" from numbers to region names
...
# fill in missing values for variable "zip" according to their "borough" values
...
# create new variable zip3: the first 3 digits of zip number
dat$ZIP3<-substr(dat$ZIP,1,3)

# rescale (normalize) numerical variables
## calculating the avg of variables
...
#filling in missing value with the average
record<-numeric()
